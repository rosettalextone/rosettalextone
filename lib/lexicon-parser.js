...
export default class LexiconParser {
    ...
    static parseType(typeByte) {
        const parsedByte = TypeMap.get(typeByte);

        switch(parsedByte) {
            case 'Underline':
                return {type: 'underline', value: true};
            case 'DisableUnderlined':
    ...
    static parseLine(line) {
        const result = line.reduce((acc, section) => {
            const controlByte = section[0];
            const type = section[1];

            if (controlByte && !TypeMap.has(type)) {
                console.error('Unknown symbol after trigger!');
                return acc;
            }

            switch(controlByte) {
                case Control.Trigger:
                    let bytes = section.slice(2);
                    const parsedType = LexiconParser.parseType(type);

                    switch(parsedType.type) {
                        case 'underline':
                            acc.underline = parsedType.value;
                            break;
                        default:
                            acc.type = parsedType.type;
                    }
    ...
    static divideByTrigger(line) {
        ...
        return line.reduce((chunks, byte) => {
            switch(byte) {
                case Control.Trigger:
                    return chunks.concat([[Control.Trigger]]);
                default:
